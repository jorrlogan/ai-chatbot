...
export async function getInvitationByToken(token: string) {
  try {
    const [result] = await db.select().from(invitation).where(eq(invitation.token, token)).limit(1);
    return result;
  } catch (error) {
...
export async function createUser(firstName: string, lastName: string, email: string, password: string, orgId: string) {
